...
ID_FILE_PATH = '../mtg_jamendo_dataset/data/download'
...
def main_download(dataset, type, download_from, outputdir, unpack, remove):
    download(dataset, type, download_from, outputdir, unpack, remove)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='autotagging_moodtheme', choices=['raw_30s', 'autotagging_moodtheme'],
                        help='dataset to download')
    parser.add_argument('--type', default='melspecs', choices=['audio', 'audio-low', 'melspecs', 'acousticbrainz'],
    ...
    #                         'or fast MTG mirror (Finland)')
    parser.add_argument('--outputdir', default='D:/magisterka-dane', help='directory to store the dataset')
    parser.add_argument('--unpack', default=True, action='store_true', help='unpack tar archives')
    parser.add_argument('--remove', default=True, action='store_true',
                        help='remove tar archives while unpacking one by one (use to save disk space)')

    args = parser.parse_args()
    main_download(args.dataset, args.type, args.download_from, args.outputdir, args.unpack, args.remove)
